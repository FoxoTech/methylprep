...
baseDir = paste0("/Users/mmaxmeister/methylprep/docs/example_data/minfi")
# read samplesheet
...
# preprocessNoob + betas
mSet.noob = preprocessNoob(rgSet)
meth.noob = getMeth(mSet.noob)
write.csv(file=file.path(baseDir,'minfi_noob_meth.csv'),x=meth.noob,row.names=TRUE)
unmeth.noob = getUnmeth(mSet.noob)
write.csv(file=file.path(baseDir,'minfi_noob_unmeth.csv'),x=unmeth.noob,row.names=TRUE)
betas.noob = getBeta(mSet.noob)
write.csv(file=file.path(baseDir,'minfi_noob_betas.csv'),x=betas.noob,row.names=TRUE)
