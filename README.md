...
\* mouse arrays are partially supported
...
```
Note: All the same input parameters from command line apply to `run_pipeline`, except `--all`. Type `help(methylprep.run_pipeline)` to see more details. 
...
Use `help(methylprep.make_pipeline)` for details.
...
- `process` is the main function for processing methylation data from `idat` files. (See above)
